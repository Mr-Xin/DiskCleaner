...
| 版本 | 范围 | 状态 |
|---|---|---|
| **v1.0** 当前 | 主要 5 屏 + 次要 3 屏 + 5 个状态屏 + 智能清理中心 + 清理过渡动画 = **14 屏** | 已完成 |
| **v1.0 补完** | 外接磁盘主页 · 首启引导 · 删除/卸载确认弹窗 · 浅色模式 | 已完成（4 屏 + 主题切换） |
| **v1.1** Power User | ⌘K 命令面板 · 保存搜索 · 键盘快捷键速查 · 自定义扫描规则 | 已完成（4 屏） |
| **v1.2** 信心与可控感 | 废纸篓恢复 · 清理历史 · 调度日历 · 通知中心 | 已完成（4 屏） |
| **v1.3** 系统集成 | 菜单栏 mini-app · 桌面 Widget · Apple Shortcuts · iCloud 偏好同步 | 已完成（4 屏） |
| v2.0+ | AI 洞察 · 场景预设 · 跨设备视图 · 趋势预测 · 家庭账户 | 待设计 |

**总计：30 张高保真稿**，按章节分布在 `DiskFlow Design.html` 的 7 个 DCSection 中。
...
下面是 v1.0 → v1.3 中每一屏的设计要点，对应 `DiskFlow Design.html` 中第 04 ~ 07 节。

### 04 · v1.0 补完（发版必做）

| 屏 ID | 名称 | 关键设计 |
|---|---|---|
| `external-drives` | 外接磁盘主页 | 2 列网格展示已连接驱动器 · 每卡含图标 + 名称 + 标签（"启动盘"/"Time Machine"）+ 用量大数 + 进度条 + 「扫描此盘」CTA；底部单独一行展示"最近离线"驱动器 |
| `onboarding` | 首启引导 | 2 列布局：左品牌大字 + 3 个特性介绍；右步骤指示器（欢迎 → 授权 → 首次扫描）+ 当前步骤卡 + CTA「打开系统设置授权」 |
| `confirm-delete` | 删除 > 1 GB 确认 | 在虚化的大文件页背景上弹出 440px 居中卡 · 警告色图标 + 文件清单预览 + 「不再确认」复选 + 取消/移到废纸篓双按钮 |
| `confirm-uninstall` | 完全卸载确认 | 480px 卡 · 应用大图标 + 5 行待删除项目清单（按风险高亮 "应用支持目录" 行）+ 黄色警告条 +「保留应用支持目录」复选 + 双按钮 |
| **浅色模式** | 主题切换 | 通过 Tweaks 面板的「主题」单选切换 · 所有屏幕同步切换 · CSS 变量级覆盖 |

### 05 · v1.1 · Power User

| 屏 ID | 名称 | 关键设计 |
|---|---|---|
| `palette` | ⌘K 命令面板 | 620px 浮层 · 顶部输入栏带闪烁光标 + 「推荐操作 / 导航 / 动作 / 最近搜索」4 个分组 · 高亮选中行带左侧蓝色边条 · 底部 ↑↓ Tab 提示栏 |
| `saved-searches` | 保存搜索 | 大文件页变体 · 左侧 240px 列出 5 个已保存查询（点色、命中数）+ 「最近搜索」二级列表；主区显示当前命中的结构化查询结果 + 「订阅命中通知」按钮 |
| `shortcuts` | 键盘快捷键 | 720px 浮层 · 2×2 分组（导航 / 动作 / 选择 / 清理）· 每行 keycap 用 mono 字体加边框框起来 |
| `scan-rules` | 自定义扫描规则 | 设置内页 · 3 个区块（扫描根目录 · 排除规则 · 高级）· 排除规则每行带类型 chip（glob/path/type）+ 显示原因 · 支持「从 .gitignore 导入」 |

### 06 · v1.2 · 信心与可控感

| 屏 ID | 名称 | 关键设计 |
|---|---|---|
| `trash-recovery` | 废纸篓恢复 | 按时间分组（今天 / 昨天 / 12 天前 即将过期 ⚠）· 每项可单独恢复或永久删除 · 顶部容量条 + 安全提示卡 |
| `cleanup-history` | 清理历史 | 月度柱状图（每天释放量，>5GB 显示数值）+ 左 1/3 分类汇总 + 右 2/3 最近清理列表（支持撤销） |
| `schedule` | 调度日历 | 周视图日历 · 时间网格按 1 小时单位 · 不同任务类型用颜色区分（蓝=快速扫描 / 紫=深度 / 绿=智能清理 / 青=报告）· 当前时间指示线带发光小球 |
| `notifications` | 通知中心 | 从顶栏铃铛位置弹出的 420px 抽屉 · 按"今天/昨天/更早"分组 · 未读项左侧蓝色光点 · 每项可带 inline action |

### 07 · v1.3 · 系统集成

| 屏 ID | 名称 | 关键设计 |
|---|---|---|
| `menubar` | 菜单栏 mini-app | 在仿 macOS 桌面背景上展示 340px 弹出窗 · 含磁盘条 + 内存条 + 智能清理 CTA + 4 个快捷动作 |
| `widget` | 桌面 Widget | 同样仿桌面背景 · 展示 280×320 中号（大数字 312 GB + 分段进度条 + 12.4 GB 可释放）+ 160×160 小号（内存环形图） |
| `shortcuts-int` | Apple Shortcuts 集成 | 介绍页 + 6 个可用动作（查询 / 动作）+ 3 个示例自动化 + CLI 命令展示 |
| `icloud` | iCloud 偏好同步 | 账户头部 + 6 项可勾选同步项目（带尺寸）+ 已链接设备列表（标识"当前设备"/"需更新"）+ 端到端加密说明 |

### Tweaks 调节面板

页面右下角的 Tweaks 面板（toolbar 切换）目前提供：
- **主题** 单选：深色 / 浅色 —— 切换后所有 30 屏同步切换 CSS 变量

后续可以接入更多 tweak（字号、间距、强调色等），不必改设计稿本身。
